...
from selenium.common.exceptions import ElementNotInteractableException, ElementNotVisibleException, NoSuchElementException
...
try:
    ...
    hddn_date_script = 'document.querySelector("{0}").value = "{2}"; document.querySelector("{1}").value = "{2}";'.format(hddn_date_input_selector, txt_data_input_selector, cur_date)
    print(hddn_date_script)

    date_input = browser.execute_script(hddn_date_script)

    view_report_btn_xpath = '//*[@id="btnSubmit1"]/div[2]'
    view_report_btn = browser.find_element(By.XPATH, view_report_btn_xpath)
    view_report_btn.click()
    assert('Daily Trends in FPI Investments upto $cur_date', cur_date) in\
        browser.find_element(By.XPATH, '//*[@id="dvArchiveData"]/table[1]/tbody/tr[1]/th').get_attribute('innerHTML')
